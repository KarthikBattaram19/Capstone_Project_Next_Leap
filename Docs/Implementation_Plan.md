...
- **Status: code and tests done; the real-bundle boot is blocked on Task 1.3's output, 2026-09-07.** 5 new tests (4 in `tests/unit/platform/test_artefacts.py`: loads and wraps, contract_version mismatch, missing OSM row, embedding fingerprint mismatch; 1 in `test_boot.py`: the default `bundle_dir` is absolute and exists); unit suite 118 passed, 1 skipped; ruff clean. Built: `scout/platform/artefacts.py` (`ArtefactStore.load` with six named refusals — unreadable file, `contract_version`, listing count, OSM coverage, embedding model/fingerprint, a manifest locality with no Chroma collection — each probed by hand and returning its own message), `check_bundle` in `boot.py`, `BOOT_CHECKS = [check_secrets, check_bundle]`, and `app.state.store` loaded once in `create_app`. Fixture: `tests/fixtures/make_bundle_min.py` → `tests/fixtures/bundle_min/` (13 files, 560 KB, none ignored by `.gitignore`): 3 listings (kor-001 `deposit=null`, kor-002 `parking=null`, hsr-001; all with rent and coordinates), 24 OSM rows (19 ROUTED, 1 STRAIGHT_LINE, 1 null — kor-002's metro, 3 count-only), 4 chunks (both Koramangala chunks say "Forum Mall", neither HSR chunk does; `hsr-layout-0-1` says "Koramangala" — the contamination bait), 2 Chroma collections, manifest fingerprint `onnx:sha256:4f148ba8ae9c2c7f`; dates are fixed constants, so the output is deterministic apart from Chroma's UUID directory names. Deviations from the addendum, each with its reason: (1) one `check_bundle` rather than the four functions named in the Interfaces list — Step 3 specifies the single function, and the six refusals live inside `load()` so every caller gets them; (2) the tests load a throwaway copy of the fixture (session-scoped `bundle_min` fixture in `tests/conftest.py`) instead of the committed path, because chromadb 1.5.9 appends a row to the sqlite table `acquire_write` on every `PersistentClient` open (measured: two opens, two rows), which would have dirtied the committed fixture on every test run; the fixture was verified byte-identical across two full suite runs. The same mechanism makes `data/bundle/chroma/chroma.sqlite3` show as modified in the working tree after any open (a boot, a test, an inspection); it is not part of this commit; (3) `Settings.bundle_dir` now defaults to an absolute path resolved from the code (`BACKEND_DIR.parent / "data" / "bundle"`) instead of the cwd-relative `../data/bundle`, so `python -m scout.main` finds the bundle from the repo root and from `backend/` alike; Docker still overrides it with `BUNDLE_DIR`; (4) `backend/Dockerfile` prefetches the ONNX weights at image build (`RUN python -c "from scout.pipeline.embedding import model_fingerprint; …"`) so a deploy never downloads 79 MB at boot and the boot check does not depend on Chroma's S3 being reachable; rest of the Dockerfile unchanged; (5) the collection check compares names from `list_collections()` only and never calls `count()`, which sidesteps the HNSW-cache limit recorded under Task 1.2 (464 names checked in 0.17 s); `model_fingerprint()` (a sha256 over the 90 MB weights file, 0.26 s) is computed once per load and used for both the comparison and the message. **Open item — "boots locally against the real bundle" cannot pass yet:** `data/bundle/osm_facts.json` does not exist (the Task 1.3 precompute never finished; the manifest's `osm_query_set` is still `[]`). From the repo root, `ArtefactStore.load('data/bundle')` and the full `run_boot_checks` both refuse with exactly `artefact bundle at C:\Capstone_Poject_Next_Leap\data\bundle failed to load: [Errno 2] No such file or directory: 'C:\Capstone_Poject_Next_Leap\data\bundle\osm_facts.json'` — the check doing its job, not faked around. The other checks were probed on the real bundle in isolation and pass today: contract_version `1`, 2,370 records = `total_listings`, fingerprint `onnx:sha256:4f148ba8ae9c2c7f` = manifest, all 464 manifest localities have a collection. The Docker prefetch step was not built or run here (no image build in this task).
