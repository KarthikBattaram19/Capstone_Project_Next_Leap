...
| 0.9 Deploy the skeleton | **backend live**; frontend not deployed | `https://capstoneprojectnextleap-production.up.railway.app` answers `/health` and `/contract`; the deployed `/ws` answers a good hello and closes a wrong one with `4400`. Vercel and the CORS allowlist still to do |
