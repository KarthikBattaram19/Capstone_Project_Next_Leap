...
- `_km(metres: int) -> str` — computes `km = metres / 1000` and returns `f"{km:.1f} km"` when `km < 10`, otherwise `f"{km:.0f} km"` (one decimal below 10 km, whole kilometres from 10 km up).
- `render_commute(fact: Provenanced[Distance], what: str) -> CommuteRendering` — control flow:
  ...
  3. If `fact.method is Method.ROUTED`: `badge = "by route"`; `spoken` is `f"about a {d.minutes}-minute walk by route to the {what.lower()}"` when `d.minutes is not None`, else `f"about {_km(d.metres)} by route to the {what.lower()}"`. Then, if `fact.timing is Timing.LIVE`, `spoken` is replaced by `f"about {d.minutes} minutes by route"` when `d.minutes is not None`, otherwise left as already set.
...
- `_same_flat(a: ListingRecord, b: ListingRecord) -> bool` — if both `a.society_name` and `b.society_name` are truthy and `a.society_name.strip().lower() == b.society_name.strip().lower()`, returns `True`; otherwise if both `a.coordinates` and `b.coordinates` are truthy, returns whether `haversine_m(a.coordinates.lat, a.coordinates.lng, b.coordinates.lat, b.coordinates.lng) <= MERGE_RADIUS_M`; otherwise returns `False`.
...
- `_overlap(a: str, b: str, n: int = 6) -> bool`: sets `aw, bw = a.lower().split(), b.lower().split()`, builds `grams = {" ".join(bw[i:i + n]) for i in range(len(bw) - n + 1)}` (every `n`-word window of `b`), and returns `any(" ".join(aw[i:i + n]) in grams for i in range(len(aw) - n + 1))` — true when some `n`-word window of `a` also occurs in `b`.
...
- a session-scoped fixture `settings` that first checks `if not os.getenv("GROQ_API_KEY") or not os.getenv("ANTHROPIC_API_KEY"):` and calls `pytest.skip("eval suites need GROQ_API_KEY and ANTHROPIC_API_KEY")`, otherwise returns `Settings(_env_file=None, cors_allowed_origins="http://localhost:3000", bundle_dir=str(FIXTURES))`;
...
- `test_barge_in_is_the_one_extra_edge()`: asserts `transition(TurnState.SPEAKING, TurnState.CAPTURING) is TurnState.CAPTURING`.
...
| `TurnState.SPEAKING` | `{TurnState.IDLE, TurnState.CAPTURING}` — comment: `CAPTURING` = barge-in (spec §6.17) |
...
The loop over `checks`: for each `(field, check)` in order, `constrained = getattr(c, field) not in (None, (), frozenset(), False)`; if not constrained, `continue`; otherwise `result = check()`; if `result is None` return `Unknown(field)`; if `result is False` return `ExcludedV(field, _reason(listing, field, c))`. So the first unmet or unknown hard field decides the verdict. If every constrained check passes, soft hits are counted: `soft = 0`; add 1 if `f("lift").value` is truthy; add 1 if `f("deposit").value is not None` and `f("rent").value` is truthy and `f("deposit").value <= 3 * f("rent").value`; return `Match(soft_hits=soft)`.
...
- `async def text(self, text: str) -> None` (comment: `typed fallback (spec §6.13)`) — sets `self._segments = [text]` and calls `await self._finalize()`.
...
- `async def _speech_started(self) -> None` — if `self.state is TurnState.SPEAKING` (comment: `barge-in (spec §6.17)`): `await self.orch.cancel_speech(self.session)`; if `self._turn` exists and is not done, `self._turn.cancel()`; then `self.state = transition(self.state, TurnState.CAPTURING)`. Else if `self.state is TurnState.IDLE`: `self.state = transition(self.state, TurnState.CAPTURING)`. In every case, `self._speech_started_at = self._speech_started_at or time.monotonic()` (records the first speech-start time only).
...
- `class ClaimAssembler`:
  ...
  - `bind(self, s: Job2Sentence) -> BoundClaim | None` — if `not s.fact_refs`, returns `None`. Takes `known = self._b.all_refs()`; if any `r` in `s.fact_refs` is not in `known`, returns `None`. Builds `facts = {r: (self._b.facts.get(r) or self._chunks[r]) for r in s.fact_refs}`. If every `f.value is None` across `facts.values()` and `_ASSERTS_VALUE.search(s.text)` matches, returns `None` (comment: a value asserted for a fact we do not have). Otherwise returns `BoundClaim(text=s.text, refs=list(s.fact_refs), facts=facts)`.
...
- Resolves the listing: `lid = session.focus_listing_id or (session.last_read_order[0] if session.last_read_order else None)`. If `lid is None`, returns early with `NeedsInput(question="Which listing do you mean?", field="reference", spoken="Which listing do you mean?")`.
...
- `ReconcileQueue` — a `@dataclass` with fields `calendar: object` and `jobs: list[Job] = field(default_factory=list)`.
  ...
  - `async run_once(self) -> None`: iterates over a copy, `for j in list(self.jobs):`; inside a `try`, if `j.op == "insert"` it awaits `self.calendar.insert(j.calendar_id, **j.payload)`, otherwise it awaits `self.calendar.delete(j.calendar_id, j.payload["event_id"])`; on success it removes the job with `self.jobs.remove(j)`; on any `Exception` it leaves the job in place and increments `j.attempts += 1`.
