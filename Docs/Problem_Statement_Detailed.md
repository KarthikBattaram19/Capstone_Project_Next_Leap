...
- **The locality set is not pinned in advance.** It is whatever the supplied spreadsheet actually carries; as supplied that is 566 localities over 9,180 rows. The final locality list and the resulting total listing count are an **output of the first deliverable (§9.1)**, documented after curation — not assumed here. **Curated result (Gate D, re-decided 2026-09-05): 2,370 listings over 464 localities; 128 localities sit at the 10 ceiling and 336 below it. The per-locality counts are in `data/bundle/manifest.json` and `data/GATE_D.md`**. The 2026-09-02 figure was 3,216 over 566; the re-supplied sheet added an `availability_status` column marking 4,648 of its 9,180 rows unavailable, and those rows are now dropped at curation
...
- Imported **once at deployment** → static dataset of **up to 10 listings per locality**; the total is at most 10 × (number of localities the spreadsheet carries), fixed and documented at import time — **as curated: 2,370 listings over 464 localities (`data/GATE_D.md`)**
...
- **Availability filtering (updated 2026-09-05):** the supplied spreadsheet **does** carry an availability marker — the `availability_status` column, `Yes` on 4,532 of its 9,180 rows and `No` on 4,648, never blank. Curation drops the rows marked unavailable, so every record in the bundle is available; 102 localities hold no available row and fall out entirely (`data/GATE_D.md`). Thereafter availability lives in the in-memory overlay of §3.14, which now starts from a published value instead of "not stated". **The rule is unchanged and still written for any source: only an explicit *unavailable* drops a record; a null marker — a source that publishes none — is kept and rendered as *not stated*, never treated as unavailable.** One caveat carried from `data/SOURCE_NOTES.md`: these values were **randomly generated**, an independent coin flip per row. The mechanism is real and testable end to end; the values are not facts about any real property, and no demo may present them as such
- **The 2026-09-02 position, for the record:** the sheet then carried no marker at all, `availability_status` was null on every record, and no listing could be excluded on that basis. Gate D was decided under those terms first and re-decided when the sheet was re-supplied
...
- **Gate D outcome (re-decided 2026-09-05):** the supplied spreadsheet carries 16 of the 24 fields above. `maintenance_charges`, `maintenance_included`, `floor`, `lift`, `parking`, `amenities` and `available_from` are null on every record, and `area_basis` is `unknown` on every record (`data/GATE_D.md`). On 2026-09-02 the count was 14 of 23 and `availability_status` was in the missing list; the re-supplied sheet publishes it, and `society_type` was added to the schema because the sheet carries that column too. They stay in the filter vocabulary and render as *not stated*; a must-have on any of them places every listing in the "unknown on this filter" group below, and Suite A's per-field case for each asserts exactly that
...
### 3.2 PII
- Owner/agent names, phone numbers **and government identifiers** are removed **before** data touches the dataset, UI, logs, **or voice transcripts**
- **What the supplied sheet actually holds (since 2026-09-05):** three columns of invented-but-realistic personal data — `Name`, `Phone Number` (nine digits, stored as text) and `Voter ID` (ten mixed-case letters), each unique per row across all 9,180 rows
- **How they are kept out, in three layers.** (1) The importer reads only columns named in an **allow-list**; the three above are absent from it, and asking for one raises rather than returning a value, so a leak fails a test instead of needing a reviewer to notice. (2) Every text value that does reach a record passes a stripper that removes emails, Indian mobile numbers **and any bare nine- or ten-digit run** — the last because this sheet's numbers are nine digits and the mobile pattern never matched them. (3) The committed bundle is checked for personal data immediately before it is written, and the write is refused if any is found. The check reports counts and the file name only: printing the leaked value would republish it into the log
- `ListingRecord` also rejects unknown keys outright, so a future sheet cannot smuggle a `Name` column through by adding it
...
- **Shortlist cards** — **locality shown as the card's primary label** (with society name beneath it where stated), rent, **deposit and maintenance** (so total cost is visible up front), BHK type, size in sq ft, floor, parking, furnishing, key amenities; fields the source didn't state render as *"not stated"*, never blank and never zero; expandable
  ...
  - **Gate D outcome (2026-09-02):** with the supplied spreadsheet, maintenance, floor, lift, parking kind and listed amenities read *not stated* on every card; the rows stay, so the gap is visible rather than hidden. Parking shows the yes/no `parking_available` value with the kind marked not stated
...
|---|---|---|---|---|
| L0 | **First feedback** (both types) | **<1.8 s** *(was <300 ms; renegotiated at Gate L, 2026-09-06)* | A word spoken → that word visible in the live transcript, with the listening state shown | Deepgram interim-result latency plus one network hop. **Independent of end-of-speech**, so it has no cut-off cost — this is the number that makes the system *feel* instant |
| L1 | **Acknowledgment** (both types) | **<2.0 s** *(was <700 ms; renegotiated at Gate L, 2026-09-06)* | End-of-speech → final transcript rendered **and** processing indicator visible | Deepgram's endpointing silence window (see P3) plus one network hop. Nothing else fits in this budget — no LLM call may sit inside it. **Bounded below by P3:** the system cannot know speech has ended before the silence window has elapsed, so L1 is never traded down by shortening that window |
| L2 | **First audio — Type A** | **≤3.5 s** *(was ≤1.5 s; renegotiated at Gate L, 2026-09-06)* | End-of-speech → first TTS audio byte plays | L1 + Job 1 round trip + TTS time-to-first-byte |
| L3 | **First audio — Type B** | **≤3.5 s** *(was ≤1.5 s; renegotiated at Gate L, 2026-09-06)* | End-of-speech → first TTS audio byte plays | L1 + retrieval + a **code-built opener from facts already resolved (P8)** + TTS. Job 2's cross-provider first token no longer sits inside this budget. **Without P8 the honest figure is 2.5 s** (L1 + retrieval + Job 2 time-to-first-token + TTS) — which is why P8 is a precondition, not a nicety |
| L4 | **Shortlist rendered** | **<5 s** *(was <3 s; renegotiated at Gate L, 2026-09-06)* | End-of-speech → full shortlist rendered in the UI | Job 1 extraction. Filtering itself is application code over a few hundred records — microseconds — **provided P5 holds**. Tightened from 5 s because no LLM performs the filtering |
| L5 | **Explanation rendered** | **≤8 s** *(was ≤6 s; renegotiated at Gate L, 2026-09-06)* | Question end → **full explanation text and its citations rendered in the UI**. Explicitly **not** the end of audio playback | Job 2 generation length. Loosened from 4 s: cross-provider generation with citations cannot be held to a budget written for local Groq inference |
...
**Renegotiated at Gate L (2026-09-06; `data/GATE_L.md`).** L0–L5 were re-set from the first measurements on the deployed US service: 25 runs per turn type with a real Type A recording (Type B synthesised, provisional). Every original row missed, and not because of the models — Job 1 took 622 ms median and Job 2's first token 1,285 ms. Three terms the original table did not price in: **~1.75 s of end-of-speech detection** (the turn fires on Deepgram `UtteranceEnd` so a mid-sentence breath does not split a sentence — a measured 1.1 s breath split 4 of 4 times at a 1,000 ms endpoint; this is a deliberate deviation from P3 as worded), **~0.9 s of TTS first byte** from a US region, and **~1.3 s to Deepgram's first interim**. The new values are component medians plus headroom, not this run's maxima; the 2× rule is unchanged. Cold start measured separately: first turn L1 3,150 ms, +1.9 s over warm. **Phase 2 is obliged to try to reclaim time on three fronts**, each measured before and after: (a) end-of-speech — `speech_final` at a 1,000 ms endpoint saves 0.4 s if the false end-of-speech count (0/20 on one speaker so far) holds across more speakers, and the orchestrator may merge a late second final into the running turn instead of cancelling it; (b) region — a Singapore service would cut every provider round trip by ~200 ms, a comparison skipped by decision and reopenable; (c) session reuse in the spike so P2 is in force when the numbers are re-taken. L6–L8 are unmeasured by the skeleton and unchanged.
...
Every shortlisted property maps to a dataset record that is not marked unavailable (as of 2026-09-05 the supplied spreadsheet does carry an `availability_status` marker, and records marked unavailable are dropped at curation, §3.1); every neighborhood claim carries a citation resolving to the correct listing-scoped source; uncertainty explicitly stated where data is missing. Mix: covered neighborhoods (5), partial/no coverage — must declare gaps (5), commute verification against the disclosed method — at least one case per method in §2.3's table (5), safety/amenity claims incl. one injection-content case and **two cross-locality contamination probes — one between adjacent localities (e.g. Koramangala / HSR Layout), one between distant ones** (5). Suite C cases are stratified across at least 3 localities.
