...
def test_wrong_contract_version_closes_with_named_reason():
    code, reason = _close_info({"type": "hello", "contract_version": "0"})
...
def test_non_hello_first_frame_is_rejected():
    ...
    code, reason = _close_info({"type": "audio"})
    assert code == CLOSE_CONTRACT_MISMATCH
    assert reason == "contract_version_mismatch"


def test_the_right_hello_is_answered_and_the_socket_stays_open():
